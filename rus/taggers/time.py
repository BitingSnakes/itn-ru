...
from rus.taggers.ordinal import OrdinalFst
...
class TimeFst(GraphFst):
    ...
    Finite state transducer for classifying time
        e.g. седьмой час пять минут" -> time { hours: "07" minutes: "05" }
        e.g. половина десятого -> time { hours: "09" minutes: "30" }
        e.g. четверть одиннадцатого -> time { hours: "10" minutes: "15" }
        e.g. без четверти одиннадцатая -> time { hours: "10" minutes: "45" }
        e.g. пять минут двенадцатого -> time { hours: "11" minutes: "05" }
    ...
    def __init__(self, cardinal: CardinalFst, ordinal: OrdinalFst):
        ...
        # The ordinal tables emit zero-padded hours. The 24th hour is valid in
        # relative phrases such as "половина двадцать четвертого" (23:30),
        # but not as an ordinary HH:MM hour value.
        ...
        graph_half_hour = pynini.closure(pynutil.delete("в "), 0, 1) + pynutil.delete("половина ") + hours_ordinal
        ...
        graph_to_quarter_hour = pynutil.delete("четверть ") + to_hour_graph
        graph_to_quarter_hour = pynutil.insert('hours: "') + graph_to_quarter_hour + pynutil.insert('" minutes: "15"')

        graph_from_quarter_hour = pynutil.delete("без четверти ") + to_hour_graph
        ...
        minutes_to_hour = cardinal.graph_up_to_hundred_component + delete_space + pynutil.delete(minutes)
        graph_minutes_to_hour = pynutil.insert('minutes: "') + minutes_to_hour + pynutil.insert('"')

        # пять минут двенадцатого -> time { hours: "11" minutes: "05" }
        graph_mh = graph_minutes_to_hour + pynutil.insert(">>") + pynutil.delete(" ") + pynutil.insert(' hours: "') + to_hour_graph + pynutil.insert('"')
        ...
        # Structured day periods make 12-hour phrases unambiguous.
        ...
        graph_period = pynini.union(
            pm_hours + optional_hour_unit + delete_space + pynutil.delete(pynini.union("дня", "вечера")),
            am_hours + optional_hour_unit + delete_space + pynutil.delete(pynini.union("ночи", "утра")),
        )
        ...
        time_zone = pynini.string_map(
            [
                ...
                ("по киевскому времени", "Europe/Kyiv"),
