...
bool IsPythonWhitespace(uint32_t codepoint) {
  ...
  // str.strip()/str.split() in rus.utils.separate_punctuation.
...
uint32_t LowercaseForRussian(uint32_t codepoint) {
  // The exported lexical grammars use ASCII and Russian Cyrillic. Original
  ...
  // The Cyrillic supplement range includes Ё (U+0401 -> U+0451).
...
std::string LowercaseForClassification(const std::string& token) {
  ...
  for (size_t i = 0; i < token.size();) {
    ...
    AppendUtf8(LowercaseForRussian(codepoint), &lower);
  }
  return lower;
}
...
std::string CanonicalizeOrthography(
  ...
  const std::string decomposed_uppercase_yo =
      "\xD0\x95\xCC\x88";  // Е + U+0308
  const std::string decomposed_lowercase_yo =
      "\xD0\xB5\xCC\x88";                         // е + U+0308
  const std::string uppercase_yo = "\xD0\x81";      // Ё
  const std::string lowercase_yo = "\xD1\x91";      // ё
  std::string result;
  size_t start = 0;
  while (start <= text.size()) {
    ...
    // Match the Russian-specific NFC composition used by Python without
    // pulling a full Unicode normalization dependency into the runtime.
    ReplaceAll(&canonical, decomposed_uppercase_yo, uppercase_yo);
    ReplaceAll(&canonical, decomposed_lowercase_yo, lowercase_yo);
...
// re-attached after verbalization (mirrors rus.utils.separate_punctuation /
// attach_punctuation). Hyphens and apostrophes are word-internal in
// Russian and must not be split. UTF-8 sequences: « = C2 AB, » = C2 BB,
// … = E2 80 A6.
...
// Reproduces rus.utils.reorder: within each `tokens ` chunk, a tagger may
